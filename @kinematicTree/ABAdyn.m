function qdd = ABAdyn(ktObj, q, qd, tau, V0, V0d, Fext)
% TODO
...
% tau:  istantaneous joint action (forces/moments)
    ...
    Mtilde = nan(6, 6, ktObj.Nb);
    Mbar   = nan(6, 6, ktObj.Nb);
    btilde = nan(6, ktObj.Nb);
    bi     = nan(6, ktObj.Nb);
    ...
    for i = ktObj.Nb:-1:1
        ...
        for j = 1:length(m)
           ...
           Mtilde(:,:,i) =  Mtilde(:, :, i) + ...
                        ADstar * Mbar(:, :, children) * AdGinv(:, :, children);
           btilde(:,i) =  btilde(:, i) + ADstar * btilde(:, children) - ...
                          ADstar * Mbar(:, :, children) * a(:, :, children) * ...
                                AdGinv(:, :, children) * twists(:, i+1)+...
                          (1 ./ (X(:, children).' * Mtilde(:, :, children) * ...
                                X(:, children))) * ADstar * Mtilde(:,:,children) *...
                                X(:, children) * ...
    ...
    for i = 1:ktObj.Nb
        ...
        qdd(i) = (tau(i) - X(:, i).' * ...
            (Mtilde(:, :, i) * (VlambdaD - a(:, :, i) * Vlambda) + btilde(:, i))) ./ ...
            (X(:, i).' * Mtilde(:, :, i) * X(:, i));
